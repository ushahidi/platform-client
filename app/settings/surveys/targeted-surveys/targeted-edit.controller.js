...
function (
    ...
    $scope.finalNumbers = {
            goodNumbers: [],
            goodNumberString: '',
            //maybe better to just array.toString() rather than maintain a separate string
            badNumbers: '',
            repeatCount: 0,
            storageObj: {},
            badNumberCount: 0,
            originalCount: 0
            //remove blank elements from array and from count
            //how do we do counting as we keep adding to the total?????
        }
    ...
    function runValidations(numbers) {
        ...
        numbersArray.forEach((number) => {
            //remove symbols
            //if has letters, then put in bad number count
            let cleanNumber = number.replace(/[^0-9]+/g, '');
            let length = cleanNumber.length
            if (length > 0 && length < 5 || length > 19) {
                ...
                if ($scope.finalNumbers.storageObj[cleanNumber]) {
                    ...
                    $scope.finalNumbers.goodNumberString = $scope.finalNumbers.goodNumberString + cleanNumber + ','
                }
            }
        })

        //if it's an empty element, remove it. do we count it in the total count?
        console.log($scope.finalNumbers)
    }
    ...
    function completeStepTwo() {
        ...
        // if there are repeats 
            // indicate number of repeats removed -- total or in that run? --  where?!
        else {
